...
import { Plus, RefreshCw, MoreHorizontal, X, Loader2, Bot, Send, CheckCircle2, XCircle } from 'lucide-react';
...
export default function WecomBotsPage() {
  ...
  return (
    ...
    >
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-[100] px-4 py-2 rounded-lg shadow-lg text-white text-[12px] font-medium ${toast.type === 'success' ? 'bg-emerald-600' : 'bg-red-500'}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-slate-200 shadow-sm flex-shrink-0">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-medium text-slate-700">企微机器人管理</span>
          <span className="text-slate-400">·</span>
          <span className="text-slate-400">{bots.length} 个机器人</span>
        </div>
        <div className="flex items-center space-x-2">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 z-10">
              <tr>
                <th className="py-2 px-3 font-medium text-slate-500 min-w-[160px]">机器人名称</th>
                <th className="py-2 px-3 font-medium text-slate-500 min-w-[280px]">Webhook 地址</th>
                ...
                <tr key={bot.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-2 px-3 text-center">
                    ...
                    >
                      ...
                      ) : testResult[bot.id] === 'ok' ? (
                        <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                      ) : testResult[bot.id] === 'fail' ? (
                  ...
                  <td className="py-2 px-3 text-center relative" onClick={e => e.stopPropagation()}>
                    ...
                    {openMenuId === bot.id && (
                      <div className="absolute right-8 top-1/2 -translate-y-1/2 w-32 bg-white border border-slate-200 shadow-lg rounded-md py-1 z-40">
                        <button onClick={() => openEdit(bot)} className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-slate-700">编辑</button>
                        <button onClick={() => handleToggle(bot)} className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-slate-700">
                          {bot.isActive ? '停用' : '启用'}
                        </button>
                        <div className="h-px bg-slate-100 my-0.5 mx-2" />
                        <button onClick={() => handleDelete(bot.id)} className="w-full text-left px-3 py-1.5 hover:bg-red-50 text-red-600">删除</button>
                      </div>
                    )}
                  </td>
                </tr>
          ...
          <div className="relative w-full sm:max-w-[400px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-slate-50/50">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">配置企业微信 Webhook 机器人</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              <div>
                ...
                <input
                  ...
                  className="w-full h-8 px-2.5 border border-slate-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-[12px]"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-8 px-2.5 border border-slate-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-[12px] font-mono"
                />
