...
CREATE TABLE IF NOT EXISTS `crm_opp_actions` (
  `id` char(36) NOT NULL DEFAULT (uuid()),
  `opportunity_id` char(36) NOT NULL COMMENT '商机ID',
  `stage_id` char(36) NOT NULL COMMENT '阶段ID',
  `action_template_id` char(36) NOT NULL COMMENT '动作模板ID',
  `action_code` varchar(50) NOT NULL COMMENT '动作代码（冗余，便于查询）',
  `action_name` varchar(255) NOT NULL COMMENT '动作名称（冗余）',
  `action_type` varchar(50) NOT NULL COMMENT '动作类型（冗余）',
  `status` varchar(20) NOT NULL DEFAULT 'PENDING' COMMENT '状态：PENDING, IN_PROGRESS, DONE, SKIPPED',
  `execution_role` varchar(50) DEFAULT NULL COMMENT '执行角色',
  `executor_id` char(36) DEFAULT NULL COMMENT '执行人ID',
  `executor_name` varchar(100) DEFAULT NULL COMMENT '执行人姓名（冗余）',
  `action_data` json DEFAULT NULL COMMENT '表单数据（JSON格式）',
  `file_oss_key` varchar(500) DEFAULT NULL COMMENT 'OSS文件路径',
  ...
  `updated_by` char(36) DEFAULT NULL COMMENT '更新人',
  PRIMARY KEY (`id`),
  KEY `ix_opportunity_id` (`opportunity_id`),
  KEY `ix_stage_id` (`stage_id`),
  KEY `ix_status` (`status`),
  KEY `ix_executor_id` (`executor_id`),
  KEY `ix_opp_stage` (`opportunity_id`,`stage_id`),
  KEY `ix_action_template_id` (`action_template_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机动作日志表';
...
CREATE TABLE IF NOT EXISTS `crm_opportunity_item_deliverables` (
  `id` char(36) NOT NULL,
  `item_id` char(36) NOT NULL,
  `rule_id` char(36) DEFAULT NULL COMMENT '可选：关联是哪个规则产出的交付物',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='服务项交付成果库';
CREATE TABLE IF NOT EXISTS `crm_opportunity_item_logs` (
  `id` char(36) NOT NULL,
  `item_id` char(36) NOT NULL,
  `action_type` varchar(50) NOT NULL COMMENT 'ASSIGN, SUBMIT, APPROVE, DELIVER',
  `operator_id` char(36) NOT NULL,
  `content` text COMMENT '操作详情',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_item_id` (`item_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='流转审计日志';
CREATE TABLE IF NOT EXISTS `crm_opportunity_item_submissions` (
  `id` char(36) NOT NULL,
  `item_id` char(36) NOT NULL COMMENT '关联的服务项ID',
  `rule_id` char(36) NOT NULL COMMENT '关联的产品规则ID',
  `rule_name_snapshot` varchar(100) DEFAULT NULL COMMENT '规则名称快照',
  `is_required` tinyint(1) DEFAULT '1' COMMENT '当时是否必填',
  `sort_order` int DEFAULT '0' COMMENT '排序',
  `status` enum('pending','reviewing','ready','rejected') DEFAULT 'pending' COMMENT '收集状态',
  `review_comments` text COMMENT '审核意见',
  `file_url` varchar(512) DEFAULT NULL COMMENT 'OSS文件地址',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_item_id` (`item_id`),
  KEY `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='服务项材料审核表';
CREATE TABLE IF NOT EXISTS `crm_opportunity_items` (
  `id` char(36) NOT NULL COMMENT '服务项唯一ID(UUID)',
  `opportunity_id` char(36) NOT NULL COMMENT '关联商机ID',
  `product_id` char(36) NOT NULL COMMENT '关联原始产品ID',
  `product_snapshot` json DEFAULT NULL COMMENT '产品快照(名称,SKU,类目等)',
  `quantity` int NOT NULL DEFAULT '1' COMMENT '数量',
  `unit_price` decimal(15,2) DEFAULT '0.00' COMMENT '成交单价',
  `currency` varchar(10) DEFAULT 'IDR' COMMENT '币种',
  `executor_id` char(36) DEFAULT NULL COMMENT '执行负责人ID',
  `assigned_at` datetime DEFAULT NULL COMMENT '指派时间',
  `item_status` enum('PENDING','QUOTING','EXECUTING','DELIVERED','CANCELLED') DEFAULT 'PENDING' COMMENT '流转状态',
  `is_materials_ready` tinyint(1) DEFAULT '0' COMMENT '办理材料是否全部就绪',
  `total_rules_count` int DEFAULT '0' COMMENT '总约束项数量',
  `ready_rules_count` int DEFAULT '0' COMMENT '已通过审核的数量',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_opportunity_id` (`opportunity_id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商机服务明细表';
CREATE TABLE IF NOT EXISTS `crm_pipeline_stages` (
  `id` char(36) NOT NULL DEFAULT (uuid()),
  `stage_code` varchar(50) NOT NULL COMMENT '阶段代码（P1-P9）',
  `name` varchar(255) NOT NULL COMMENT '阶段名称',
  `order` int NOT NULL COMMENT '阶段顺序（1-9）',
  `dependency_id` char(36) DEFAULT NULL COMMENT '依赖阶段ID（必须完成的前置阶段）',
  `can_parallel` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否支持并行执行',
  `description` text COMMENT '阶段描述',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `ux_stage_code` (`stage_code`),
  KEY `ix_stage_order` (`order`),
  KEY `ix_dependency` (`dependency_id`),
...
CREATE TABLE IF NOT EXISTS `opportunities` (
  ...
  `quote_audit_status` varchar(20) DEFAULT 'PENDING' COMMENT '报价审核状态：PENDING, APPROVED, REJECTED',
  ...
  `first_payment_at` datetime DEFAULT NULL COMMENT '首次回款时间',
  `final_payment_at` datetime DEFAULT NULL COMMENT '尾款回款时间',
  `description` text COMMENT '描述',
  `organization_id` char(36) NOT NULL COMMENT '组织ID（数据隔离）',
  `created_by` char(36) DEFAULT NULL COMMENT '创建人ID',
  `updated_by` char(36) DEFAULT NULL COMMENT '更新人ID',
  ...
  KEY `ix_executor_id` (`executor_id`),
  KEY `ix_quote_audit_status` (`quote_audit_status`),
  KEY `fk_opportunities_current_stage` (`current_stage_id`),
  CONSTRAINT `fk_opportunities_current_stage` FOREIGN KEY (`current_stage_id`) REFERENCES `crm_pipeline_stages` (`id`) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS `sys_stage_actions_template` (
  `id` char(36) NOT NULL DEFAULT (uuid()),
  `stage_id` char(36) NOT NULL COMMENT '关联阶段ID',
  `action_code` varchar(50) NOT NULL COMMENT '动作代码（唯一标识）',
  `action_name` varchar(255) NOT NULL COMMENT '动作名称',
  `action_type` varchar(50) NOT NULL COMMENT '动作类型：FORM, FILE_UPLOAD, APPROVAL, SYSTEM',
  `is_required` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否必需（1=必需，0=可选）',
  `execution_role` varchar(50) DEFAULT NULL COMMENT '执行角色：SALES, FINANCE, EXECUTOR, ADMIN',
  `order` int NOT NULL DEFAULT '1' COMMENT '执行顺序',
  `validation_rules` json DEFAULT NULL COMMENT '验证规则（JSON格式）',
  `description` text COMMENT '动作描述',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `ux_stage_action` (`stage_id`,`action_code`),
  KEY `ix_stage_id` (`stage_id`),
  KEY `ix_action_type` (`action_type`),
  KEY `ix_is_active` (`is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='阶段动作模板表';
